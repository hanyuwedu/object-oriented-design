    ...
    public class BookingSystem {
        ...
        public List<Hotel> searchHotel(SearchHotelRequest request)
        {
            ...
            for (Hotel hotel : this.hotels) {
                Map<RoomType, List<Room>> rooms = hotel.handleSearchResult(searchRequest);

                int leftTotal = rooms.entrySet().stream()
                        .mapToInt(x -> x.getKey().getCapacity() * x.getValue().size())
                        .sum();

                if (leftTotal >= request.getGroupSize()) {
                    hotels.add(hotel);
                }
            }
