package jiuzhang.coffeemaker;

public class CoffeeMakerOODesign {
    public class CoffeeMaker {

        public Coffee makeCoffee(CoffeePack pack) {
            Coffee coffeeBase = new SimpleCoffee();

            Coffee coffee = coffeeBase;

            for (int i = 1; i <= pack.getNeededMilk(); i++) {
                coffee = new WithMilk(coffee);
            }

            for (int j = 1; j <= pack.getNeededSugar(); j++) {
